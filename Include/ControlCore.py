...
class CControlCore:
    ...
    역할:
        1. PyRT-style multiprocessing process 생성/관리
        2. FSM 상태 관리
        3. 각 프로세스의 Queue feedback 처리
        4. Pipe command 전송
        5. RobotTask에서 계산된 cmd_vel을 Stella B2 프로세스로 전달
    """

    EMERGENCY_CHECK_INTERVAL_SEC = 1.0
    ...
    def __init__(self):
        ...
        self._last_emergency_request_time = 0.0
        ...
        self._mp_robot_task = None
        self._mp_stella_b2 = None
    ...
    def create_processes(self):
        ...
        self._mp_stella_b2 = CMultiProcessBase(PROC_STELLA_B2, proc_stella_b2)
    ...
    def on_tts_feedback(self, msg: dict):
        ...
        if msg_type == MSG_TYPE_STATUS:
            ...
            if status == "TTS_DONE":
                ...
                elif self._state == RobotState.EMERGENCY:
                    pass
        ...
        if msg_type == MSG_TYPE_EMERGENCY:
            ...
            if is_emergency:
                self.handle_event(
                    ...
                    {
                        KEY_EMERGENCY_TYPE:
                            data.get(KEY_EMERGENCY_TYPE, data.get("emergency_type", None)),

                        KEY_CONFIDENCE:
                            data.get(KEY_CONFIDENCE, data.get("confidence", 0.0)),
    ...
    def handle_cmd_vel_message(self, msg: dict):

        linear_x = msg.get(KEY_LINEAR_X, None)
        angular_z = msg.get(KEY_ANGULAR_Z, None)
        ...
        write_log(
            "CMD_VEL | source=%s | linear_x=%.3f | angular_z=%.3f"
            % (
                str(msg.get(KEY_SOURCE, None)),
                linear_x,
                angular_z
            ),
            self
    ...
    def handle_event(self, event, data: dict = None):
        ...
        if event == RobotEvent.EMERGENCY_DETECTED:
            self.change_state(RobotState.EMERGENCY)
    ...
    def request_emergency_check(self):
        ...
        if curr_time - self._last_emergency_request_time < self.EMERGENCY_CHECK_INTERVAL_SEC:
            return
        ...
        self._last_emergency_request_time = curr_time
