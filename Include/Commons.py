...
def check_argument_type(obj, arg): # PySignal 내부에서 사용하는 타입 검사 함수
    if type(obj) == arg:
        return True
    if obj is None:
        return True
...
class PySignal(object): # PyRT 스타일의 시그널 클래스 (콜백 함수 등록 및 호출 기능), 무슨 일이 발생했으니 등록된 함수를 실행해줘
    _args = None
    _callback = None
